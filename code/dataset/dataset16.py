...
#        Mask_new = np.zeros((1, D, H, W), np.int16)
        ...
        if self.num_classes == 1:
            ...
            Mask_new = np.where((MASK == 1), 0, MASK)
            for i in range(len(ver)):    
                Mask_new = np.where((Mask_new == ver[i]), 0, Mask_new)
            for j in range(len(IVDs)):
                Mask_new = np.where((Mask_new == IVDs[j]), 1, Mask_new)
        else:
            Mask_new = (MASK != 0)
        ...
        if D > 16:
            start_z = random.choice([i for i in range(D - 16 + 1)])
            MR = crop(MR, start=start_z, end=start_z + 16, axis='z')
            spine_heatmap = crop(spine_heatmap, start=start_z, end=start_z + 16, axis='z')
            heatmaps = crop(heatmaps, start_z, end=start_z + 16, axis='z')
            MASK = crop(MASK, start=start_z, end=start_z + 16, axis='z')
            Mask_new = crop(Mask_new, start=start_z, end=start_z + 16, axis='z')
        elif D < 16:
            MR_ex = np.zeros((_,16,H,128))
            Mask_ex = np.zeros((_,16,H,128))
            start = (16 - D) // 2
            
            MR_ex[:,start:start+D,:,:] = MR
            Mask_ex[:,start:start+D,:,:] = Mask_new
            
            MR = MR_ex
            Mask_new = Mask_ex
#            for i in range(start):
#                MR_ex[:,i,:,:] = MR[:,0,:,:]
#                Mask_ex[:,i,:,:] = Mask_new[:,0,:,:]
