...
if __name__ == '__main__':
    heatmap_generator = HeatmapGenerator(image_size=(12, 256, 256),
                                             sigma=2,
                                             spine_heatmap_sigma=1,
                                             scale_factor=1.,
                                             normalize=True,
    ...
    dict_images = landmark_extractor(landmarks)
    spine_heatmap = heatmap_generator.generate_spine_heatmap(list_landmarks=dict_images)
    print(spine_heatmap.shape)
    heatmaps = heatmap_generator.generate_heatmaps(list_landmarks=dict_images)
    print(heatmaps.shape)
    import matplotlib.pyplot as plt
    b = np.sum(heatmaps, axis=1)
    for k in range(19):
#        plt.matshow(spine_heatmap[0,k,:,:], cmap=plt.cm.gray)
#        plt.show()
        plt.matshow(b[k,:,:], cmap=plt.cm.gray)
        plt.show()
