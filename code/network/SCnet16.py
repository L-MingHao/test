...
class Deconv3D_Block(nn.Module):
    def __init__(self, in_channels, out_channels, kernel=4, stride=2, padding=1):
        ...
        self.deconv = Sequential(
                        ConvTranspose3d(in_channels, out_channels, kernel_size=kernel,
                                stride=stride, padding=(padding, padding, padding), output_padding=0,
                                bias=True),
                        ReLU()
                        )
...
class SpatialConfigurationNet(nn.Module):
    def __init__(self,
        ...
        # self.local_heatmaps = Sequential(
        #                             Conv3d(in_channels=channels, out_channels=num_labels, kernel_size=(1,1,1)),
        #                             Sigmoid()
        #                             )
        ...
        ## verse 2020 unet
        # self.scnet_spatial = UNet3D(in_channels=num_labels, num_classes=num_labels, filters=self.num_filters_base)
        # self.spatial_heatmaps = Conv3d(in_channels=num_labels, out_channels=num_labels, kernel_size=(1,1,1))
        self.upsampling = Upsample(scale_factor=(1, self.downsampling_factor, self.downsampling_factor), mode='trilinear', align_corners=True)
...
#        # spatial part
#        node = self.downsampling(node)

#        # verse 2019
#        node = self.conv1(node)
#        node = self.conv2(node)
#        node = self.conv3(node)
#        node = self.conv4(node)


#        ## verse 2020
#        # node = self.scnet_spatial(node)
#        # node = self.spatial_heatmaps(node)
#        spatial_heatmaps = self.upsampling(node)
...
if __name__=='__main__':
    ...
    net = SpatialConfigurationNet(num_labels=1, residual='pool')
#    writer = SummaryWriter()
    x = torch.ones(1, 1, 16, 64, 64)
    print(x.size())
