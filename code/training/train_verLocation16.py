...
MODEL = {'3dUnet':SpatialConfigurationNet, 'ResUnet':ResUNet}
# 解析参数
def parse_args():
    ...
    # model info 
    parser.add_argument('--model', default='3dUnet')
    parser.add_argument('--input_channels', default=12, type=int,
                        help='input channels')
    parser.add_argument('--num_classes', default=1, type=int,
                        help='number of classes')
    parser.add_argument('--input_d', default=16, type=int,
                        help='image depth')
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, landmark, _ in train_loader:
        input = input.cuda()
        target = target.cuda()
        ...
        for i in range(config['num_classes']):
            iou += (iou_score(output[i,:,:,:], target[0,i,:,:,:]))
...
def validate(config, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for input, target, landmark, _  in val_loader:
            input = input.cuda()
            target = target.cuda()
...
#                                    )
    model = MODEL[config['model']](num_labels=config['num_classes'])
    model = model.cuda()
...
#    img_names = [os.path.splitext(os.path.basename(p))[0] for p in img_names]
    ...
    train_dataset = Dataset(img_names=train_img_names,
                            img_dir='../dataset/RawData/',
                            transform=train_transform,
                            num_classes = config['num_classes'],
                            train = True)
    val_dataset = Dataset(img_names=val_img_names,
                        img_dir='../dataset/RawData/',
