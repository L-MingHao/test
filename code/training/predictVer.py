...
def main():
    ...
    with torch.no_grad():
        for input, target, landmark_, meta in tqdm(val_loader, total=len(val_loader)):
            ...
            np.save('../outputs/{}/{}/PredVerAll.npy'.format(config['name'], meta['img_name'][0]), FinalOutput)
...
#                print(y)
            ...
            cv2.imwrite(os.path.join('../outputs', config['name'], meta['img_name'][0], 'RealVer' + '.bmp'),
                            FInalImgM, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
            map = heat_map(PosAll, 256, 128)
            cv2.imwrite(os.path.join('../outputs', config['name'], meta['img_name'][0], 'VersHeatmap' + '.bmp'),
                            map, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
    print('IoU: %.4f' % avg_meter.avg)
    np.save('../outputs/{}/LocVer.npy'.format(config['name']), hashtable)
