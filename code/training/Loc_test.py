...
#                    print(np.mean(a[:,1]) - np.mean(b[:,1]))
        ...
        save.to_csv("../outputs/testdata/{}/PredPoints.csv".format(img_names[index]),index=True,header=True)  #index=False,header=False表示不保存行索引和列标题
        ...
        for i in range(len(d)+1):
            ...
            if pointNums != 0 and pointNums > 150:
                ...
                ymean //= pointNums
                xmean += add
            cv2.rectangle(ver_mask_img, (max(0, int(ymean-2*h)), max(0, int(xmean-2*w))), (min(128,int(ymean+2*h)), min(256, int(xmean+2*w))), 128 , 2) 
        save = pd.DataFrame(dVer, columns = ['x', 'y']) 
        cv2.imwrite("../outputs/testdata/{}/PredVer-Box.bmp".format(img_names[index]), ver_mask_img)
        save.to_csv("../outputs/testdata/{}/PredPointsVer.csv".format(img_names[index]),index=True,header=True)  #index=False,header=False表示不保存行索引和列标题
