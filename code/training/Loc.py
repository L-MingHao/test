# -*- coding: utf-8 -*-
"""
Created on Fri May 21 14:38:25 2021
...
if __name__ == "__main__":
    ...
    addVer = 2
    var = addIVDs - addVer
    hashtable = np.load('../outputs/data/Loc.npy', allow_pickle=True).item()
    hashtableHeat = np.load('../outputs/data/LocPos.npy', allow_pickle=True).item()
    hashtableVer = np.load('../outputs/data/LocVer.npy', allow_pickle=True).item()
...
#    img_names =  ["Case12"]
    for index in range(len(img_names)):
        ...
        # Ivds
        #######################################################################
        binary_img = cv2.imread("../outputs/data/{}/PredIVDs.bmp".format(img_names[index]), cv2.IMREAD_GRAYSCALE)
        mask_img = cv2.imread("../outputs/data/{}/RealIVDs.bmp".format(img_names[index]), cv2.IMREAD_GRAYSCALE)
...
#                    print(np.mean(a[:,1]) - np.mean(b[:,1]))
                    if  len(points[0]) < len(points[1])- 150 and np.mean(a[:,1]) - np.mean(b[:,1]) > 10 and len(points[0]) < 120:
                        continue
                    
#                    print(len(points[0]))
#                    print(len(points[1])-160)
#                    if np.mean()
#                    if len(points[0]) < len(points[1])- 160 and (xmax - xmin) / (ymax - ymin) >  0.6:
#                        continue
                num += 1
                xmean = 0
                ymean = 0
                for j in range(len(points[i])):
                    ...
                    ymean += y
                    binary_img[x,y] = 255
                d.append([xmean/len(points[i])+addIVDs, ymean/len(points[i])+hashtableHeat[img_names[index]]-64])
                xmean //= len(points[i])
                ymean //= len(points[i])
                cv2.rectangle(mask_img, (max(0, ymean-h), max(0, xmean-w)), (min(128,ymean+h), min(256, xmean+w)), 255, 2)     
        print("IVDs:{}".format(num))
        hashtable[img_names[index]][1] = num
        save = pd.DataFrame(d, columns = ['x', 'y']) 
        cv2.imwrite("../outputs/data/{}/PredIVDs-Box.bmp".format(img_names[index]), mask_img)
        save.to_csv("../outputs/data/{}/PredPoints.csv".format(img_names[index]),index=True,header=True)  #index=False,header=False表示不保存行索引和列标题
        
        #######################################################################
        # Ver
        #######################################################################
        ver_binary_img = cv2.imread("../outputs/data/{}/PredVer.bmp".format(img_names[index]), cv2.IMREAD_GRAYSCALE)
        ver_mask_img = cv2.imread("../outputs/data/{}/RealVer.bmp".format(img_names[index]), cv2.IMREAD_GRAYSCALE)
            ...
            for px in range(len(temp)):
                for py in range(len(temp[0])):
                    if temp[px][py] == 255:
                        ...
                        ymean += py
                        pointNums += 1
            if i == len(d) and pointNums <= 150:
                temp = ver_binary_img[int(d[i-1][0])-addIVDs-3:min(256,int(d[i-1][0]-addIVDs)+3),80:]
                add = int(d[i-1][0])
                pointNums = 0
                xmean = 0
                ymean = 0
                ...
                if pointNums > 90:
                    num += 1
                    dVer.append([xmean/pointNums+addVer+add, ymean/pointNums+hashtableHeat[img_names[index]]-64])
                    xmean //= pointNums
                    ymean //= pointNums
                    xmean += add
                    cv2.rectangle(ver_mask_img, (max(0, int(ymean-h)), max(0, int(xmean-w))), (min(128,int(ymean+h)), min(256, int(xmean+w))), 255, 2) 
            elif pointNums > 150:
                num += 1
                dVer.append([xmean/pointNums+addVer+add, ymean/pointNums+hashtableHeat[img_names[index]]-64])
                xmean //= pointNums
        ...
        print("Ver:{}, real:{}".format(num, hashtableVer[img_names[index]][0]))
        hashtableVer[img_names[index]][1] = num
        save = pd.DataFrame(dVer, columns = ['x', 'y']) 
        cv2.imwrite("../outputs/data/{}/PredVer-Box.bmp".format(img_names[index]), ver_mask_img)
        save.to_csv("../outputs/data/{}/PredPointsVer.csv".format(img_names[index]),index=True,header=True)  #index=False,header=False表示不保存行索引和列标题
    np.save('../outputs/data/LocVer.npy', hashtableVer)
    np.save('../outputs/data/Loc.npy', hashtable)
    
    error = 0
    for i in hashtable:
        if hashtable[i][0] != hashtable[i][1]:
            error += 1
            if i in val_img_names:
                print("Test {}: {}-{}".format(i, hashtable[i][0], hashtable[i][1]))
            else:
                print("Train {}: {}-{}".format(i, hashtable[i][0], hashtable[i][1]))
    print(error)


    error = 0
    for i in hashtableVer:
        if hashtableVer[i][0] != hashtableVer[i][1]:
            error += 1
            if i in val_img_names:
                print("Test {}: {}-{}".format(i, hashtableVer[i][0], hashtableVer[i][1]))
            else:
                print("Train {}: {}-{}".format(i, hashtableVer[i][0], hashtableVer[i][1]))
    print(error)