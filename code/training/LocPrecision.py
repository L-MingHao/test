...
#    img_names.remove("Case91")
    ...
    for target in ['x', 'y']:
        ...
        for index in range(len(img_names)):
            ...
            for i in range(len(predIvds)):
                if target == 'x':
                    hashtableVer[img_names[index]][i+10] = [predIvds[target][i], 0]
                else:
                    hashtableVer[img_names[index]][i+10][1] = predIvds[target][i]
        ...
        for i in range(10):
            train_x, val_x = train_test_split(train[i], test_size=0.3, random_state=40)
    ...
    for i in hashtableVer:
        ...
        save.columns = ['x', 'y']
        save.to_csv('../outputs/data/{}/Predfinal.csv'.format(i),index=True,header=True)
