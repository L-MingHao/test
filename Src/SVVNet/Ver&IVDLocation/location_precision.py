    ...
    for target in ['x', 'y']:
        ...
        for index in range(len(img_names)):
            ...
            for i in range(len(pred)):
                ...
                if i == len(pred) - 1:
                    if i < len(predIvds) and not np.isnan(real[target][i]) and not np.isnan(
                            real[target][i]) and not np.isnan(predIvds[target][i - 1]) and not np.isnan(
                            predIvds[target][i]):
                    ...
                    elif i == 9 and not np.isnan(real[target][i]) and not np.isnan(real[target][i]) and not np.isnan(
                            predIvds[target][i - 2]) and not np.isnan(predIvds[target][i - 1]):
                        x[i].append([pred[target][i], predIvds[target][i - 2], predIvds[target][i - 1]])
                        hashtable[img_names[index]][i] = [pred[target][i], predIvds[target][i - 2],
                                                          predIvds[target][i - 1]]
                        y[i].append(real[target][i])
                elif i == 0:
                    if not np.isnan(real[target][i]) and not np.isnan(real[target][i]) and not np.isnan(
                            predIvds[target][i]) and not np.isnan(predIvds[target][i + 1]):
                        ...
                        y[i].append(real[target][i])
                elif not np.isnan(real[target][i]) and not np.isnan(real[target][i]) and not np.isnan(
                        predIvds[target][i - 1]) and not np.isnan(predIvds[target][i]):
                    x[i].append([pred[target][i], predIvds[target][i - 1], predIvds[target][i]])
                    hashtable[img_names[index]][i] = [pred[target][i], predIvds[target][i - 1], predIvds[target][i]]
                    y[i].append(real[target][i])
        ...
        for index in range(len(test_names)):
            ...
            for i in range(len(pred)):
                ...
                if i == len(pred) - 1:
                    if i < len(predIvds) and not np.isnan(predIvds[target][i - 1]) and not np.isnan(
                            predIvds[target][i]):
                        hashtabletest[test_names[index]][i] = [pred[target][i], predIvds[target][i - 1],
                                                               predIvds[target][i]]
                    elif i == 9 and not np.isnan(real[target][i]) and not np.isnan(real[target][i]) and not np.isnan(
                            predIvds[target][i - 2]) and not np.isnan(predIvds[target][i - 1]):
                        hashtabletest[test_names[index]][i] = [pred[target][i], predIvds[target][i - 2],
                                                               predIvds[target][i - 1]]
                elif i == 0:
                    if not np.isnan(predIvds[target][i]) and not np.isnan(predIvds[target][i + 1]):
                        hashtabletest[test_names[index]][i] = [pred[target][i], predIvds[target][i],
                                                               predIvds[target][i + 1]]
        ...
        for j in range(10):
            if hashtableVer[i][j] != []:
                ...
                for k in range(len(hashtableVerZ)):
                    temp += (k) * np.sum(hashtableVerZ[k, max(0, int(hashtableVer[i][j][0] - 36)):min(256, int(
                        hashtableVer[i][j][0] + 36)),
                                         max(0, int(hashtableVer[i][j][1] - 36 - hashtableHeat[i] + 32)):min(128, int(
                                             hashtableVer[i][j][1]) + 36 - hashtableHeat[i] + 32)])
                print(np.sum(
                    hashtableVerZ[:, max(0, int(hashtableVer[i][j][0] - 36)):min(256, int(hashtableVer[i][j][0] + 36)),
                    max(0, int(hashtableVer[i][j][1] - 36 + hashtableHeat[i] - 128)):min(128, int(
                        hashtableVer[i][j][1]) + 36 + hashtableHeat[i] - 128)]))
                hashtableVer[i][j].append(temp / np.sum(
                    hashtableVerZ[:, max(0, int(hashtableVer[i][j][0] - 36)):min(256, int(hashtableVer[i][j][0] + 36)),
                    max(0, int(hashtableVer[i][j][1] - 36 - hashtableHeat[i] + 32)):min(128, int(
                        hashtableVer[i][j][1]) + 36 - hashtableHeat[i] + 32)]))
        ...
        for j in range(10, 19):
            if hashtableVer[i][j] != []:
                ...
                for k in range(len(hashtableIVDsZ)):
                    temp += (k) * np.sum(hashtableIVDsZ[k, max(0, int(hashtableVer[i][j][0]) - 36):min(256, int(
                        hashtableVer[i][j][0]) + 36),
                                         max(0, int(hashtableVer[i][j][1]) - 36 - hashtableHeat[i] + 32):min(128, int(
                                             hashtableVer[i][j][1]) + 36 - hashtableHeat[i] + 32)])
                hashtableVer[i][j].append(temp / np.sum(
                    hashtableIVDsZ[:, max(0, int(hashtableVer[i][j][0]) - 36):min(256, int(hashtableVer[i][j][0]) + 36),
                    max(0, int(hashtableVer[i][j][1]) - 36 - hashtableHeat[i] + 32):min(128, int(
                        hashtableVer[i][j][1]) + 36 - hashtableHeat[i] + 32)]))
