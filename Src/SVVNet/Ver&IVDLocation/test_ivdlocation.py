...
from DataLoader.dataloader_IVDlocation import landmark_extractor
...
def inference(trainer, list_case_dirs, save_path, do_TTA=False):
    ...
    with torch.no_grad():
        ...
        for case_dir in tqdm(list_case_dirs):
            ...
            if not os.path.exists('../../../Output/IVD_Location/' + case_id):
                os.mkdir('../../../Output/IVD_Location/' + case_id)
            np.save('../../../Output/{}/{}/PredVerAll.npy'.format('IVD_Location', case_id), FinalMask)
            FinalImgO = np.zeros((256, 128))
            for k in range(len(temp)):
                FinalImgO += FinalMask[k, :, :]
            FinalImgO = (FinalImgO != 0)
            FinalImgO = (FinalImgO * 255).astype('uint8')
            cv2.imwrite(os.path.join('../../../Output', 'IVD_Location', case_id, 'PredVer' + '.bmp'),
