...
class TrainerLog:
    def __init__(self):
        ...
        # Average iou of a epoch
        self.average_iou = 0.
        self.best_average_iou = 0.
        ...
        # Record changes in validation evaluation iou
        self.list_average_iou_associate_iter = []
...
class NetworkTrainer:
    ...
    def set_optimizer(self, optimizer_type, args, params):
        ...
        if optimizer_type == 'SGD':
            self.setting.optimizer = optim.SGD(params, lr=args['lr'], momentum=args['momentum'],
    ...
    # Added iou score update
    def update_iou(self, iou):
        self.log.average_iou = iou
        if iou > self.log.best_average_iou:
            self.log.best_average_iou = iou
            self.log.save_status.append('best_val_evaluation_iou')
        self.log.list_average_iou_associate_iter.append([self.log.average_iou, self.log.iter])
    ...
    def run(self):
        ...
        while (self.log.epoch < self.setting.max_epoch - 1) and (self.log.iter < self.setting.max_iter - 1):
            ...
            self.print_log_to_file(
                '            Average val evaluation iou is   %12.12f,     best is           %12.12f\n'
                % (self.log.average_iou, self.log.best_average_iou), 'a')
